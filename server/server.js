...
app.post('/api/auth/login', async (req, res, next) => { try { const u = await User.findOne({ email: req.body.email?.trim().toLowerCase() }); if (!u || !(await bcrypt.compare(req.body.password || '', u.password))) return res.status(401).json({ message: 'Invalid email or password.' }); res.json({ token: makeToken(u), user: publicUser(u) }); } catch (e) { next(e); } });
...
mongoose.connect(mongoUri).then(async () => { for (const email of ['sandeeptrangarajan@gmail.com', 'yogabalan2007yoga@gmail.com']) { if (!await User.findOne({ email })) await User.create({ email, password: bcrypt.hashSync('Admin@ksrce', 10), role: 'admin' }); } app.listen(process.env.PORT || 5000, () => console.log('MongoDB API running')); }).catch((e) => { console.error('MongoDB connection failed:', e.message); process.exit(1); });
